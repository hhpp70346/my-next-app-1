...
const MENU_ITEMS = {
  issue_manager: [
    ...
    },
    {
      ...
      icon: faLock,
      path: "/issue-manager/change-password",
...
export default function IssueManagerLayout({ children }) {
  ...
  const [screenName, setScreenName] = useState("نظام إدارة الإصدار");
  ...
  useEffect(() => {
    const authenticate = async () => {
      ...
      if (userData.role !== "issue_manager") {
        router.push("/");
        return;
      }
  ...
  useEffect(() => {
    ...
    const roleMenu = MENU_ITEMS[user.role] || [];
    let currentScreen = "نظام إدارة الإصدار";
