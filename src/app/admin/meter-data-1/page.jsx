...
import { 
  FaPaintBrush, FaSave, FaSyncAlt, FaSearch, FaEraser, FaEdit, FaTimes, 
  FaIdCard, FaTachometerAlt, FaCalendarAlt, FaUser, FaCheckSquare, FaBell, 
  FaCheckCircle, FaUserPlus, FaUserEdit, FaCoins, FaExclamationTriangle, 
  FaStickyNote, FaChartLine, FaBalanceScale, FaMoneyBillWave, FaPercent,
...
const MeterData1 = () => {
  ...
  const getValueByColumn = (item, columnIndex) => {
    const columnsMap = {
      ...
      22: parseFloat((item.totalSettlement || 0) - (item.paidDiscount || 0)),
  ...
  const calculateTotals = (data) => {
    ...
    data.forEach(item => {
      ...
      totals.netSettlement += (parseFloat(item.totalSettlement || 0) - parseFloat(item.paidDiscount || 0));
  ...
  const applyFilters = () => {
    return data.filter(item => {
      return (
        ...
        (filters.permissionNumber === '' || item.permissionNumber?.toLowerCase().includes(filters.permissionNumber.toLowerCase())) &&
        (filters.meterNumber === '' || item.meterNumber?.toLowerCase().includes(filters.meterNumber.toLowerCase())) &&
        ...
        (filters.accountReference === '' || item.accountReference?.toLowerCase().includes(filters.accountReference.toLowerCase())) &&
        (filters.intentional === '' || item.intentional == (filters.intentional === '1')) &&
        (filters.billNotice === '' || item.billNotice == (filters.billNotice === '1')) &&
        (filters.success === '' || item.success == (filters.success === '1')) &&
  ...
  return (
    <div className="container">
      <h2><FaEdit className="fa-icon" /> استعلام تعديل عن فحص أحادي الوجه</h2>
      <div className="filter-container">
        ...
        <div className="form-row">
          <button id="applyFilters" type="button" onClick={() => setData(applyFilters())}>
              ...
              <table id="dataTable">
                ...
                <tbody id="tableBody">
                  {currentRecords.map((item, index) => {
                    ...
                    const netSettlement = (item.totalSettlement || 0) - (item.paidDiscount || 0);
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>{parseFloat(item.balanceTo || 0).toFixed(2)}</td>
                        <td>{parseFloat(item.balanceDebtor || 0).toFixed(2)}</td>
                        <td>{item.meterStatus}</td>
                        <td>{item.faultType}</td>
                        <td>{item.faultNotes}</td>
                        <td>{item.intentional ? "نعم" : "لا"}</td>
                        <td>{item.billNotice ? "نعم" : "لا"}</td>
                        <td>{parseFloat(item.averageQuantity || 0).toFixed(2)}</td>
                        ...
                        <td>{faultDays}</td>
                        <td>{parseFloat(item.settlementQuantity || 0).toFixed(2)}</td>
                        <td>{parseFloat(item.totalSettlement || 0).toFixed(2)}</td>
                        <td>{parseFloat(item.paidDiscount || 0).toFixed(2)}</td>
                        ...
                        <td>{item.branch}</td>
                        <td>
                          <button onClick={() => openEditModal(item.id)} className="edit-btn">
                ...
                <div className="form-row">
                  ...
                  <div className="form-group checkbox">
                    <input 
                      id="intentional" 
                      type="checkbox" 
                      checked={currentEditData.intentional || false}
                  ...
                  <div className="form-group checkbox">
                    <input 
                      id="billNotice" 
                      type="checkbox" 
                      checked={currentEditData.billNotice || false}
                  ...
                  <div className="form-group checkbox">
                    <input 
                      id="success" 
                      type="checkbox" 
                      checked={currentEditData.success || false}
