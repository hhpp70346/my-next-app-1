...
import { 
  ...
  FaCheckCircle, FaUserPlus, FaUserEdit, FaUserSlash, FaPrint,
...
export default function DeletedRecords() {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/meter-data-5');
      if (!response.ok) {
  ...
  const applyFiltersLogic = (data) => {
    ...
    return data.filter(item => {
      ...
      if (deletedByFilter !== "" && item.restored_by !== deletedByFilter) return false;
      return true;
    });
  };
  ...
  const uniqueDeletedBy = [...new Set(data.map(item => item.restored_by).filter(Boolean))];
    ...
    <div className="container">
      ...
      <div className="filter-container">
        ...
        <div className="form-row">
          ...
          <button id="printReport" onClick={printReport}>
            <FaPrint className="fa-icon" /> طباعة التقرير
          </button>
        </div>
      </div>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      'القائم بالإضافة', 'القائم بالتعديل', 'القائم بالاسترجاع', 'الفرع التابع له',
                    ].map((header, index) => (
      ...
      <div id="printableArea" style={{ display: 'none' }}>
        <img src="/11.png" alt="شعار الشركة" className="logo" style={{ width: '150px', position: 'absolute', top: '20px', left: '20px' }} />
        <div className="company-name" style={{ position: 'absolute', top: '20px', right: '20px', fontSize: '20px', fontWeight: '600', color: 'var(--primary)' }}>
          الشركة المصرية للطاقة المتجددة
        </div>
        <h2 style={{ textAlign: 'center', marginTop: '150px', color: 'var(--primary)', fontWeight: '600' }}>تقرير فحص أحادي الوجه</h2>
        <div className="print-info" style={{ textAlign: 'left', marginTop: '20px', fontSize: '14px', color: 'var(--primary)' }}>
          <p><strong>القائم بالطباعة:</strong> {user?.username || 'غير معروف'}</p>
          <p><strong>الفرع:</strong> {user?.branch || 'غير معروف'}</p>
        ...
        <div className="date-range" style={{ textAlign: 'center', marginTop: '20px', fontSize: '16px', fontWeight: '600', color: 'var(--primary)' }}>
          <p>تاريخ الطلب من: {filters.requestDateFrom || 'غير محدد'} إلى: {filters.requestDateTo || 'غير محدد'}</p>
        </div>
        <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0', margin: '50px auto 0', fontSize: '12pt' }}>
          <thead>
            <tr>
              <th>#</th>
              <th>رقم الاذن</th>
