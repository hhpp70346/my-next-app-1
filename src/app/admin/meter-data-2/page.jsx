...
import { 
  FaPaintBrush, FaSave, FaSyncAlt, FaSearch, FaEraser, FaEdit, FaTimes, FaIdCard, 
  FaTachometerAlt, FaCalendarAlt, FaUser, FaCheckSquare, FaBell, FaCheckCircle, 
  FaUserPlus, FaUserEdit, FaCoins, FaExclamationTriangle, FaStickyNote, FaChartLine, 
  FaBalanceScale, FaMoneyBillWave, FaPercent, FaTrash, FaStepBackward, FaStepForward, 
...
const MeterData2 = () => {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/meter-data-2');
      if (!response.ok) {
  ...
  const getValueByColumn = (item, columnIndex) => {
    const columnsMap = {
      ...
      4: item.meterNumber,
      5: parseFloat(item.balanceTo) || 0,
      6: parseFloat(item.balanceDebtor) || 0,
      7: item.meterStatus,
      8: item.faultType,
      9: item.faultNotes,
      10: item.intentional ? 1 : 0,
      11: item.billNotice ? 1 : 0,
      12: parseFloat(item.averageQuantity) || 0,
      ...
      19: parseFloat(item.settlementQuantity) || 0,
      20: parseFloat(item.totalSettlement) || 0,
      21: parseFloat(item.paidDiscount) || 0,
      22: (parseFloat(item.totalSettlement) - parseFloat(item.paidDiscount)) || 0,
  ...
  const applyFilters = () => {
    return data.filter(item => {
      return (
        ...
        (!filters.requestDateFrom || new Date(item.requestDate) >= new Date(filters.requestDateFrom)) &&
        (!filters.requestDateTo || new Date(item.requestDate) <= new Date(filters.requestDateTo)) &&
        item.accountReference.toLowerCase().includes(filters.accountReference.toLowerCase()) &&
        (filters.intentional === '' || item.intentional.toString() === filters.intentional) &&
        (filters.billNotice === '' || item.billNotice.toString() === filters.billNotice) &&
        (filters.success === '' || item.success.toString() === filters.success) &&
        ...
        (filters.restoredBy === '' || (item.restored_by && item.restored_by.toLowerCase().includes(filters.restoredBy.toLowerCase())))
      );
    });
  };
  ...
  // تصفية البيانات وتقسيمها إلى صفحات
  ...
  const uniqueRestoredBy = [...new Set(data.map(item => item.restored_by).filter(Boolean))];

  return (
    <div className="container">
      <h2>استعلام حذف عن فحص أحادي الوجه</h2>
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            <div className="input-group">
              <select 
                id="filterRestoredBy" 
                value={filters.restoredBy}
                onChange={(e) => setFilters({...filters, restoredBy: e.target.value})}
              >
                <option value="">كل</option>
                {uniqueRestoredBy.map((user, index) => (
                  <option key={index} value={user}>{user}</option>
                ))}
              </select>
              <label className="transparent-label"><FaUserEdit className="fa-icon" /> القائم بالاسترجاع:</label>
            </div>
          </div>
        </div>
        <div className="form-row">
          <button id="applyFilters" type="button" onClick={() => setCurrentPage(1)}>
      ...
      <h2><FaTrash className="fa-icon" /> حذف بيانات فحص أحادي الوجه</h2>
              ...
              <table id="dataTable">
                <thead>
                  <tr>
                    {[
                      ...
                      'القائم بالإضافة', 'القائم بالتعديل', 'القائم بالاسترجاع', 'الفرع التابع له', 'الإجراءات'
                    ].map((header, index) => (
                ...
                <tbody id="tableBody">
                  {currentRecords.map((item, index) => {
                    ...
                    const netSettlement = (parseFloat(item.totalSettlement) - parseFloat(item.paidDiscount)) || 0;
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>{item.restored_by || '-'}</td>
                        <td>{item.branch}</td>
                        <td>
                          <button onClick={() => deleteData(item.id)}>
                            <FaTrash className="fa-icon" /> حذف
                          </button>
                        </td>
                      </tr>
                    );
